...
\newpage
    ...
    In general \textbf{loss function} in NST contains two sub-losses: the content loss and the style loss. The idea is pretty straightforward. The first loss ensures that the activations of the higher layers are similar between the content image and the generated image. The second one makes sure that the correlation of activations in all the layers are similar between the style image and the generated image. 
    
    more definition here

    The concept is depicted on Figure \fig{loss-equation} where α and β are user-defined parameters. By controlling α and β one can decide how much style of source image will be preserved (and similarly - the amount of the source image content).
    \begin{figure}[h!]
        \centering
